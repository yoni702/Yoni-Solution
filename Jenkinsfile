node (label: 'slave'){
    ...
    stage('4 - Build new image based on nginx image') {
        ...
        //create a volume
        sh 'sudo docker volume create --name volume --opt type=none --opt device=/text-files/ --opt o=bind'
    ...
    stage('5 - Run container with files from Stage 3') {
        ...
        sh 'sudo docker run -d -p 80:80 --name=yoni_site  --mount source=volume,target=/var/www/html,readonly yoni_site'
